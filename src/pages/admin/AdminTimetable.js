...
const SUBJECT_COLORS = {
  ...
  "Urdu":             { color: "#f97316", bg: "#fff7ed" },
  "Science":          { color: "#0ea5e9", bg: "#f0f9ff" },
  "Social Studies":   { color: "#d97706", bg: "#fffbeb" },
  ...
  "Moral Studies":    { color: "#8b5cf6", bg: "#f5f3ff" },
};

const getSubjectStyle = (s) => SUBJECT_COLORS[s] || { color: "#6b7280", bg: "#f9fafb" };
...
export default function AdminTimetable() {
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }}>
        <div className="form-group">
          ...
          {errors.period && <div style={{ color: "#e53e3e", fontSize: 11.5, marginTop: 4, fontWeight: 600 }}>{errors.period}</div>}
        </div>
        <div className="form-group">
          ...
          {errors.subject && <div style={{ color: "#e53e3e", fontSize: 11.5, marginTop: 4, fontWeight: 600 }}>{errors.subject}</div>}
        </div>
        <div className="form-group">
          ...
          {errors.teacher && <div style={{ color: "#e53e3e", fontSize: 11.5, marginTop: 4, fontWeight: 600 }}>{errors.teacher}</div>}
        </div>
        <div className="form-group">
          ...
          {errors.room && <div style={{ color: "#e53e3e", fontSize: 11.5, marginTop: 4, fontWeight: 600 }}>{errors.room}</div>}
        </div>
      </div>
    </div>
  );
  ...
  const PeriodCard = ({ entry, day, index }) => {
    ...
    return (
      <div style={{ background: st.bg, border: `1.5px solid ${st.color}33`, borderLeft: `4px solid ${st.color}`, borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <main className="main-content">
        ...
        <div className="card" style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                {SECTIONS.map(s => (
                  <button key={s} onClick={() => setSelectedSection(s)}
                    style={{ width: 36, height: 36, borderRadius: 8, border: `2px solid ${selectedSection === s ? "#9b6dff" : "var(--border)"}`, background: selectedSection === s ? "#f5f3ff" : "#fff", color: selectedSection === s ? "#7c3aed" : "var(--text-secondary)", fontWeight: 800, fontSize: 13.5, cursor: "pointer", fontFamily: "inherit" }}>
                    {s}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
              {["week","day"].map(v => (
                <button key={v} onClick={() => setView(v)}
                  style={{ padding: "8px 16px", borderRadius: 8, border: "1.5px solid var(--border)", background: view === v ? "#9b6dff" : "var(--bg-main)", color: view === v ? "#fff" : "var(--text-secondary)", fontWeight: 700, fontSize: 12.5, cursor: "pointer", fontFamily: "inherit" }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 14, flexWrap: "wrap" }}>
            <span style={{ background: "#f5f3ff", color: "#7c3aed", fontSize: 12, fontWeight: 700, padding: "4px 12px", borderRadius: 20 }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
            {DAYS.map(d => (
              <button key={d} onClick={() => setActiveDay(d)}
                style={{ flex: 1, padding: "10px 8px", borderRadius: 10, border: `2px solid ${activeDay === d ? "#9b6dff" : "var(--border)"}`, background: activeDay === d ? "#f5f3ff" : "#fff", color: activeDay === d ? "#7c3aed" : "var(--text-secondary)", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>
                {DAY_SHORT[d]}
                <div style={{ fontSize: 10.5, fontWeight: 700, marginTop: 2, color: activeDay === d ? "#9b6dff" : "var(--text-muted)" }}>{getDay(d).length} periods</div>
              </button>
            ))}
          </div>
        )}
              ...
              <div key={day}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                  ...
                  <button onClick={() => openAdd(day)}
                    style={{ background: "#f5f3ff", border: "1.5px solid #e9d5ff", borderRadius: 8, padding: "4px 8px", cursor: "pointer", color: "#7c3aed", fontWeight: 700, fontSize: 11.5, display: "flex", alignItems: "center", gap: 3, fontFamily: "inherit" }}>
                    <Icon d="M12 5v14M5 12h14" size={11} color="#7c3aed" /> Add
                  </button>
                </div>
                    ...
                    if (!entry) {
                      return (
                        <div key={period} style={{ border: "1.5px dashed var(--border)", borderRadius: 8, padding: "10px", textAlign: "center", cursor: "pointer", transition: "all 0.15s" }}
                          ...
                          <div style={{ fontSize: 10, color: "#c4b5fd", fontWeight: 700, marginTop: 2 }}>+ Add</div>
                        </div>
                      );
                    }
                    const st = getSubjectStyle(entry.subject);
                    return (
                      <div key={period} style={{ background: st.bg, border: `1.5px solid ${st.color}33`, borderLeft: `3px solid ${st.color}`, borderRadius: 8, padding: "8px 10px", cursor: "pointer", position: "relative", group: true }}
        ...
        {view === "day" && (
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            <div style={{ padding: "16px 20px", background: "linear-gradient(135deg,#9b6dff,#7c3aed)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 500, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
              ...
              <div style={{ padding: "16px 28px", borderTop: "1px solid var(--border)", display: "flex", gap: 12 }}>
                ...
                <button onClick={handleAdd} className="btn-primary" style={{ flex: 1, background: "linear-gradient(135deg,#9b6dff,#7c3aed)" }}>Add Period</button>
              </div>
            </div>
          </div>
        )}
            ...
            <div style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 500, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
              ...
              <div style={{ padding: "16px 28px", borderTop: "1px solid var(--border)", display: "flex", gap: 12 }}>
                <button onClick={() => { setEditEntry(null); setErrors({}); }}
                  style={{ flex: 1, padding: "11px", borderRadius: 10, border: "1.5px solid var(--border)", background: "#fff", color: "var(--text-secondary)", fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
                <button onClick={handleEdit} className="btn-primary" style={{ flex: 1, background: "linear-gradient(135deg,#2db87b,#1e9e63)" }}>Save Changes</button>
              </div>
            </div>
          </div>
        )}
