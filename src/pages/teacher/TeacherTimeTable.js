...
import { teacherTimetable } from "../../data/teacherMockData";
...
const SUBJECT_COLORS = {
  ...
  "Mathematics":      { color: "#d4a017", bg: "#fdf8e1", light: "#fef3c7" },
...
const CLASS_COLORS = { "10-A": "#4f8ef7", "10-B": "#9b6dff", "9-A": "#14b8a6" };
...
function TeacherTimetable() {
  ...
  const byDay = DAYS.reduce((acc, d) => {
    acc[d] = teacherTimetable.filter(t => t.day === d);
    return acc;
  }, {});

  const todaySchedule = byDay[activeDay] || [];

  const totalPeriods = teacherTimetable.length;
  const uniqueClasses = [...new Set(teacherTimetable.map(t => t.class))];
      ...
      <main className="main-content">
        ...
        <div className="stat-mini-row" style={{ gridTemplateColumns: "repeat(4, 1fr)" }}>
          {[
            ...
            { label: "Classes",       value: uniqueClasses.length,  color: "#4f8ef7", icon: "M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0-4 4v2"             },
            ...
            {todaySchedule.length === 0 ? (
              ...
              <div className="tt-period-list">
                {todaySchedule.map((item, i) => {
                  ...
                  const cc = CLASS_COLORS[item.class] || "#6b7280";
                  return (
                    <div key={i} className="tt-period-card" style={{ borderLeft: `4px solid ${s.color}`, animationDelay: `${i * 60}ms` }}>
                      ...
                      <div className="tt-period-body">
                        ...
                        <div style={{ display: "flex", gap: "8px", alignItems: "center", marginTop: "3px" }}>
                          <span style={{ padding: "2px 10px", borderRadius: "8px", background: cc + "20", color: cc, fontSize: "12px", fontWeight: 800 }}>
                            Class {item.class}
                          </span>
          ...
          <div className="card" style={{ overflowX: "auto" }}>
            <div className="card-header" style={{ marginBottom: "16px" }}>
              <div className="card-title">
                ...
                Weekly Overview — Spring 2026
              </div>
            </div>

            {/* Subject Color Legend */}
            <div style={{ display: "flex", gap: "10px", marginBottom: "14px", flexWrap: "wrap" }}>
              {[...new Set(teacherTimetable.map(t => t.subject))].map((sub, i) => {
                ...
                return Array.from({ length: maxPeriods }, (_, pi) => (
                  <React.Fragment key={pi}>
                    ...
                    {DAYS.map(d => {
                      ...
                      const cc = item ? (CLASS_COLORS[item.class] || "#6b7280") : null;
                      return (
                        <div key={d} className={`tt-grid-cell ${d === activeDay ? "tt-active-col" : ""}`}>
                          {item ? (
                            <div className="tt-week-cell-item" style={{ background: s.bg, borderLeft: `3px solid ${s.color}` }}>
                              ...
                              <div style={{ fontSize: "10px", fontWeight: 700, color: cc, marginTop: "2px" }}>Class {item.class}</div>
                              <div style={{ fontSize: "10px", color: "var(--text-muted)", fontWeight: 600 }}>{item.time}</div>
                            </div>
                          ) : (
