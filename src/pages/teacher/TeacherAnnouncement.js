...
function TeacherAnnouncements() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      (prof.assignedSections || []).forEach(s => {
        ...
        if (!seen.has(key)) seen.set(key, `${s.classId}-${s.section}`);
      });
      setSectionList(["All Classes", ...seen.values()]);
    }).catch(() => {}).finally(() => setLoading(false));
  }, []);

  const handlePost = async () => {
    if (!form.title.trim()) { setAnnError("Announcement title cannot be empty."); return; }
    if (!form.description.trim()) { setAnnError("Announcement message cannot be empty."); return; }
    setAnnError("");
    setPosting(true);
    try {
      const res = await apiCreateAnnouncement({
        ...
        tag: form.tag,
        targetClasses: form.target === "All Classes" ? [] : [form.target],
  ...
  const handleDelete = async (id, postedById) => {
    // Only allow deleting if this is the teacher's own announcement
    const postedByStr = postedById?._id || postedById;
    if (currentUser?.id && String(postedByStr) !== String(currentUser.id)) {
  ...
  const filtered = filterTag === "All"
    ? announcements
    : announcements.filter(a => a.tag === filterTag.toLowerCase());
    ...
    <div className="app-layout">
      ...
      <main className="main-content">
          ...
          <div className="card" style={{ border: "2px solid #bbf7d0", background: "linear-gradient(135deg, #f0fdf7, #fff)" }}>
              ...
              <div style={{ display: "flex", gap: "12px" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <select className="form-input" value={form.target} onChange={e => setForm(p => ({ ...p, target: e.target.value }))}>
                    {sectionList.map(c => <option key={c}>{c}</option>)}
                  </select>
                </div>
              </div>
            </div>
            ...
            <div style={{ marginTop: "14px", display: "flex", justifyContent: "flex-end", gap: "10px" }}>
              ...
              <button onClick={handlePost} disabled={posting}
                style={{ padding: "10px 28px", borderRadius: "10px", border: "none", background: posting ? "#9aaabb" : "linear-gradient(135deg, #2db87b, #1e9e63)", color: "#fff", fontWeight: 800, fontSize: "13.5px", cursor: "pointer", fontFamily: "inherit", boxShadow: "0 4px 12px rgba(45,184,123,0.35)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {loading ? <div className="card" style={{ textAlign: "center", padding: "40px", color: "var(--text-muted)" }}>Loading…</div>
          : filtered.length === 0 ? (
            ...
            return (
              <div key={a._id || i} style={{ display: "flex", background: "var(--card-bg)", borderRadius: "14px", border: "1.5px solid var(--border)", overflow: "hidden", transition: "box-shadow 0.18s, transform 0.18s", animation: `fadeSlideUp 0.35s ease ${i * 60}ms both` }}
                ...
                <div style={{ padding: "16px 20px", flex: 1 }}>
                  ...
                  <p style={{ fontSize: "13px", color: "var(--text-secondary)", lineHeight: 1.6 }}>{a.content || a.description}</p>
                </div>
                <div style={{ display: "flex", alignItems: "center", padding: "0 16px" }}>
                  {/* Only show delete for own announcements, not admin's */}
                  {(() => {
                    const postedByStr = a.postedBy?._id || a.postedBy;
                    const isOwn = currentUser?.id && String(postedByStr) === String(currentUser.id);
                    const isAdmin = a.postedBy?.role === "admin";
                    if (isAdmin) return (
                      <span style={{ fontSize: "10.5px", color: "var(--text-muted)", fontWeight: 700, padding: "4px 10px", borderRadius: "6px", background: "var(--bg-page)", border: "1px solid var(--border)" }}>
                        Admin
                      </span>
                    );
                    if (isOwn) return (
                      <button onClick={() => handleDelete(a._id, a.postedBy)}
                        ...
                        <Icon d="M18 6L6 18M6 6l12 12" size={12} color="#e53e3e" />Delete
                      </button>
                    );
                    return null;
                  })()}
