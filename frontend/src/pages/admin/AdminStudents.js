...
export default function AdminStudents() {
  ...
  const handleSubmit = () => {
    ...
    setSuccessMsg(`Student "${form.name}" added successfully.`);
  ...
  const handleEditSubmit = () => {
    ...
    if (!editForm.phone.trim() && !editForm.fatherPhone.trim()) e.fatherPhone = "At least one contact is required";
    if (!editForm.section.trim()) e.section = "Section is required";
    setEditErrors(e);
    if (Object.keys(e).length > 0) return;
    setStudents(prev => prev.map(s => s.id === editStudent.id ? { ...editForm } : s));
    setSuccessMsg(`Student record for "${editForm.name}" updated successfully.`);
    setTimeout(() => setSuccessMsg(""), 4000);
    setEditStudent(null);
    setEditForm(null);
  };
                    ...
                    <tr key={s.id} style={{ borderBottom: "1px solid var(--border)", transition: "background 0.15s" }}
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", gap: 6 }}>
                          <button onClick={() => setViewStudent(s)} style={{ background: "#f5f3ff", color: "#7c3aed", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                            View
                          </button>
                          <button onClick={() => openEdit(s)} style={{ background: "#e8f0ff", color: "#2563eb", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                            Edit
                          </button>
                          <button onClick={() => setDeleteConfirm(s.id)} style={{ background: "#fff5f5", color: "#dc2626", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                            Delete
                          </button>
                        </div>
                      </td>
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 560, maxHeight: "90vh", overflow: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
              <div style={{ padding: "24px 28px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <button onClick={() => setEditStudent(null)} style={{ background: "var(--bg-main)", border: "none", borderRadius: 8, padding: 8, cursor: "pointer" }}>
                  <Icon d="M18 6L6 18M6 6l12 12" size={16} />
                </button>
              </div>
              <div style={{ padding: "20px 28px" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }}>
                  {[["Full Name","name"],["Email Address","email"],["Phone Number","phone"],["Section","section"],["Father's Name","fatherName"],["Father's Phone","fatherPhone"],["Guardian Info","address"]].map(([label, key]) => (
                    <div key={key} className="form-group">
                      <label className="form-label">{label}</label>
                      <input className={`form-input${editErrors[key] ? " input-error" : ""}`} value={editForm[key] || ""} onChange={e => setEditForm(f => ({ ...f, [key]: e.target.value }))} />
                      {editErrors[key] && <div style={{ color: "#e53e3e", fontSize: 11.5, marginTop: 4, fontWeight: 600 }}>{editErrors[key]}</div>}
                    </div>
                  ))}
                  <div className="form-group">
              ...
              <div style={{ padding: "16px 28px", borderTop: "1px solid var(--border)", display: "flex", gap: 12 }}>
                ...
                <button onClick={handleEditSubmit} className="btn-primary" style={{ flex: 1, background: "linear-gradient(135deg,#4f8ef7,#2563eb)" }}>✓ Update Record</button>
              </div>
            </div>
          </div>
        )}
        ...
        {deleteConfirm && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div style={{ background: "#fff", borderRadius: 16, padding: "28px 32px", maxWidth: 380, textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
              ...
              <div style={{ color: "var(--text-secondary)", fontSize: 13.5, marginBottom: 22 }}>This will permanently remove the student record. The student will no longer be able to log in.</div>
