...
export default function AdminTeachers() {
  ...
  const validate = () => {
    ...
    if (step === 1) {
      ...
      else if (teachers.some(t => t.email.toLowerCase() === form.email.trim().toLowerCase()))
  ...
  const handleSubmit = () => {
    ...
    setSuccessMsg(`Teacher "${form.name}" added successfully.`);
  ...
  const handleDeleteTeacher = (id) => {
    ...
    setSuccessMsg(`Teacher "${teacher?.name}" removed successfully.`);
    setTimeout(() => setSuccessMsg(""), 4000);
              ...
              <div key={t.id} className="card" style={{ padding: "20px", display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: "auto" }}>
                  ...
                  <button onClick={() => setDeleteConfirm(t.id)}
                    style={{ padding: "8px 10px", borderRadius: 8, border: "1.5px solid #fca5a5", background: "#fff5f5", color: "#dc2626", fontWeight: 700, fontSize: 12.5, cursor: "pointer", fontFamily: "inherit" }}>
        ...
        {deleteConfirm && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div style={{ background: "#fff", borderRadius: 16, padding: "28px 32px", maxWidth: 380, textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
              ...
              <div style={{ color: "var(--text-secondary)", fontSize: 13.5, marginBottom: 22 }}>This will permanently remove the teacher record. The teacher will no longer be able to log in.</div>
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <button onClick={() => handleDeleteTeacher(deleteConfirm)} style={{ flex: 1, padding: "11px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#ff5c5c,#dc2626)", color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>Delete</button>
              </div>
            </div>
          </div>
        )}
