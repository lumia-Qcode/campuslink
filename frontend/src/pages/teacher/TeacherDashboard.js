...
function TeacherDashboard() {
    ...
    async function load() {
      try {
        const [profRes, annRes, matRes] = await Promise.all([
          apiGetMyTeacherProfile(),
          apiGetAnnouncements({ role: "teacher" }),
          apiGetMaterials(),
        ]);
        ...
        const sections = prof.assignedSections || [];
        const pairs = [...new Map(sections.map(s => [`${s.classId}|${s.section}`, { classId: s.classId, section: s.section }])).values()];
        let allStudents = [];
        for (const { classId, section } of pairs) {
          try {
            const sRes = await apiGetStudents({ classId, section });
            allStudents = [...allStudents, ...(sRes.data || [])];
          } catch (_) {}
        }
  ...
  const tagColors = { urgent: "tag-urgent", event: "tag-event", info: "tag-info", notice: "tag-notice" };
  const tagLabels = { urgent: "Urgent", event: "Event", info: "Info", notice: "Notice" };
      ...
      <main className="main-content">
        ...
        <div className="hero-banner" style={{ background: "linear-gradient(135deg, #1e9e63 0%, #2db87b 60%, #4ade80 100%)" }}>
          <div className="hero-text">
            ...
            <p>{loading ? "Loading your schedule..." : `You have ${todaySchedule.length} classes scheduled today. Stay organized!`}</p>
        ...
        <div className="stat-mini-row">
          {[
            { label: "Total Students", value: loading ? "…" : students.length,    icon: "M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8zM23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75", color: "#2db87b", bg: "#f0fdf7", iconClass: "icon-green"  },
            { label: "Classes Today",  value: loading ? "…" : todaySchedule.length, icon: "M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01",                                                                                    color: "#4f8ef7", bg: "#eff6ff", iconClass: "icon-blue"   },
            { label: "Materials",      value: loading ? "…" : materials.length,   icon: "M4 19.5A2.5 2.5 0 0 1 6.5 17H20M4 19.5A2.5 2.5 0 0 0 6.5 22H20V2H6.5A2.5 2.5 0 0 0 4 4.5v15z",                                          color: "#e6a800", bg: "#fffbeb", iconClass: "icon-yellow" },
            { label: "My Sections",    value: loading ? "…" : sections.length,    icon: "M9 11l3 3L22 4M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11",                                                               color: "#9b6dff", bg: "#f5f3ff", iconClass: "icon-purple" },
          ].map((s, i) => (
        ...
        <div className="dashboard-grid-two">
          <div className="card">
            ...
            <div style={{ display: "flex", gap: "6px", marginBottom: "14px" }}>
              {DAYS.map(d => (
                <button key={d} onClick={() => setActiveDay(d)} style={{ padding: "5px 12px", borderRadius: "8px", border: "1.5px solid", borderColor: activeDay === d ? "#2db87b" : "var(--border)", background: activeDay === d ? "#2db87b" : "var(--bg-main)", color: activeDay === d ? "#fff" : "var(--text-secondary)", fontWeight: 700, fontSize: "12px", cursor: "pointer", fontFamily: "inherit" }}>{d}</button>
              ))}
            </div>
            {loading ? <div style={{ textAlign: "center", padding: "28px", color: "var(--text-muted)", fontSize: "13px" }}>Loading…</div>
            : todaySchedule.length === 0 ? <div style={{ textAlign: "center", padding: "28px", color: "var(--text-muted)", fontSize: "13px" }}>No classes on this day</div>
            : <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                {todaySchedule.map((item, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 12px", borderRadius: "10px", background: "var(--bg-main)", border: "1.5px solid var(--border)", borderLeft: "3px solid #2db87b" }}>
                    <div style={{ padding: "4px 10px", borderRadius: "7px", background: "#f0fdf7", color: "#2db87b", fontSize: "11px", fontWeight: 800, flexShrink: 0 }}>{item.period}</div>
                    <div style={{ flex: 1 }}>
          ...
          <div className="card card-alt">
            ...
            : announcements.length === 0 ? <div style={{ color: "var(--text-muted)", fontSize: "13px" }}>No announcements yet.</div>
            : announcements.slice(0, 3).map((a, i) => {
              const tagColors2 = { urgent: "tag-urgent", event: "tag-event", info: "tag-info", notice: "tag-notice" };
              return (
                <div key={i} style={{ padding: "11px 0", borderBottom: i < 2 ? "1px solid var(--border)" : "none" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                    <span className={`tag ${tagColors2[a.tag] || "tag-info"}`}>{tagLabels[a.tag] || a.tag}</span>
                    <span style={{ fontSize: "11.5px", color: "var(--text-muted)", fontWeight: 600, marginLeft: "auto" }}>{new Date(a.createdAt).toLocaleDateString()}</span>
                  </div>
        ...
        <div className="dashboard-grid-two">
          <div className="card">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {loading ? <div style={{ color: "var(--text-muted)", fontSize: "13px" }}>Loading…</div>
              : Object.keys(classSectionGroups).length === 0 ? <div style={{ color: "var(--text-muted)", fontSize: "13px" }}>No sections assigned yet.</div>
              : Object.values(classSectionGroups).map((grp, i) => {
                ...
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: "14px", padding: "12px 14px", borderRadius: "12px", background: "var(--bg-main)", border: "1.5px solid var(--border)" }}>
                    <div style={{ width: 40, height: 40, borderRadius: "12px", background: "#f0fdf7", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span style={{ fontSize: "11px", fontWeight: 900, color: "#2db87b" }}>{label}</span>
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: "14px", fontWeight: 800, color: "var(--text-primary)" }}>Class {label}</div>
                      <div style={{ fontSize: "12px", color: "var(--text-muted)", fontWeight: 600 }}>{secStudents.length} students · {grp.subjects.join(", ")}</div>
                    </div>
                    <div style={{ display: "flex" }}>
                      ...
                      {secStudents.length > 4 && <div style={{ width: 24, height: 24, borderRadius: "50%", background: "#e4eaf0", border: "2px solid #fff", marginLeft: "-6px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "9px", fontWeight: 800, color: "var(--text-muted)" }}>+{secStudents.length - 4}</div>}
                    </div>
                  </div>
                );
              })}
          ...
          <div className="card card-alt">
            ...
            {loading ? <div style={{ color: "var(--text-muted)", fontSize: "13px" }}>Loading…</div>
            : materials.length === 0 ? <div style={{ color: "var(--text-muted)", fontSize: "13px" }}>No materials uploaded yet.</div>
            : materials.slice(0, 4).map((m, i) => {
              const tc = { PDF: "#ff5c5c", PPT: "#f5c842", DOCX: "#2db87b" }[m.fileType] || "#9b6dff";
              return (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 0", borderBottom: i < 3 ? "1px solid var(--border)" : "none" }}>
                  <div style={{ width: 36, height: 36, borderRadius: "10px", background: tc + "20", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <span style={{ fontSize: "9px", fontWeight: 900, color: tc }}>{m.fileType}</span>
                  </div>
        ...
        <div className="dashboard-grid-three">
          <div className="profile-panel" style={{ borderTop: "3px solid #2db87b" }}>
            <div className="profile-avatar-wrap">
              ...
              <div className="profile-class" style={{ color: "#2db87b" }}>{profile?.department || "—"}</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "10px" }}>
              {[
                ...
                <button key={i} onClick={() => navigate(a.path)}
                  style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "8px", padding: "14px 8px", borderRadius: "12px", border: `1.5px solid ${a.color}22`, background: a.bg, cursor: "pointer", fontFamily: "inherit", transition: "all 0.18s" }}
