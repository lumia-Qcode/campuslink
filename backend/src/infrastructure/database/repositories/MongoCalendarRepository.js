...
class MongoCalendarRepository extends ICalendarRepository {
  ...
  /**
   * Return all calendar events with optional filters.
   * Used by teacher calendar — same events as students see.
  ...
  async findAll(filters = {}) {
    ...
    let results = docs.map(d => this._toDomain(d));

    // Month filter applied in JS
    if (filters.month) {
      const monthLabel = String(filters.month);
