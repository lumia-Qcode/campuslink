 ...
 *
 * FIX: Removed .normalizeEmail() from loginValidators and signupValidators.
 * normalizeEmail() was mutating emails (e.g. stripping dots, lowercasing
 * sub-addressing) causing findByEmail() lookups to fail with 401 when the
 * stored email didn't match the normalized form. Replaced with .toLowerCase()
 * which is safe and consistent.
 */
...
const loginValidators = [
  body('email')
    ...
    // NOTE: .normalizeEmail() intentionally removed — it mutated email addresses
    // (e.g. stripping dots, lowercasing subaddressing) causing DB lookup mismatches.
    .trim()
...
const signupValidators = [
  ...
  body('email')
    .isEmail().withMessage('Valid email is required')
    // NOTE: .normalizeEmail() intentionally removed — same reason as loginValidators.
    .trim()
    .customSanitizer(value => value.toLowerCase()),
  body('password')
    .isLength({ min: 4 }).withMessage('Password must be at least 4 characters')
