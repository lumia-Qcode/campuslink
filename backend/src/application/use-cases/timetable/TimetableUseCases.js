...
class TimetableUseCases {
  ...
  async addEntry(data) {
    const { classId, section, day, period, subject, teacherId, startTime, endTime, room } = data;
    ...
    return this.timetableRepository.upsert(classId, section, day, period, {
      classId, section, day, period, subject, teacherId, startTime, endTime, room: room || '',
    });
  }
   ...
   * Receives array of entries and recreates them.
   */
  async saveTimetable(classId, section, entries) {
    ...
    for (const entry of entries) {
      const result = await this.timetableRepository.create({
        ...
        startTime: entry.startTime || entry.period.split(' - ')[0] || '',
        endTime:   entry.endTime   || entry.period.split(' - ')[1] || '',
        room:      entry.room || '',
      });
      saved.push(result);
    }
